...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['change'])) {
    ...
    if ($new !== $confirm) {
        ...
        if ($user->changePassword($_SESSION['user_id'], $old, $new)) {
            // Re‑login using the stored username from session
            $username = $_SESSION['username'] ?? '';
            if ($username) {
                $login = $user->login($username, $new);
                if ($login) {
                    $_SESSION['master_key'] = base64_encode($login['masterKey']);
                }
            }
